...
const BoxLeft = () => {
    ...
    async function funcName() {
        ...
        console.log(data);
        setchartActive_label(data.salespromocount.map(item => item.key))
        setchartActive_data(data.salespromocount.map(item => item.value))
        setchartdisActive_label(data.salespromocountInActive.map(item => item.key))
        setchartdisActive_data(data.salespromocountInActive.map(item => item.value))
    }
    if (Active) {
    ...
    const handledisActive = (e) => {
        ...
        return (
        <>
            <h6>آمار فروش بر مبنای تعداد</h6>
            {data.labels == null && data.datasets.data == null ?
                ...
                <div>
                    ...
                    {/* <Doughnut data={data} width={150} height={200} options={{ maintainAspectRatio: false }} /> */}
                </div>
            }
